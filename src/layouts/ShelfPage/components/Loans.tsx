...
export const Loans = () => {
    ...
    useEffect(() => {
        const fetchUserCurrentLoans = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `http://localhost:8083/api/books/secure/currentloans`;
    ...
    const returnBook = async (bookId: number) => {
        const url = `http://localhost:8083/api/books/secure/return?bookId=${bookId}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    const renewLoan = async (bookId: number) => {
        const url = `http://localhost:8083/api/books/secure/renew/loan?bookId=${bookId}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
